...
const App: React.FC = () => {
      ...
      try {
          ...
          if (inputType === 'Type A') {
              if (videoUrlInput.trim()) {
                  ...
                  data = { 
                      description: `Local Video File: ${videoFile?.name || "Video"}`,
                  }; 
              }
  ...
  const handleGenerate = async () => {
    if (!userApiKey) { setIsKeyModalOpen(true); return; }
    if (!analysisResult) { alert("请先点击“智能分析”按钮获取内容摘要"); return; }

    setIsGenerating(true); setResult(null);

    try {
      // Directly pass analysis result to service. 
      // Service will ONLY use this analysis + style tone to write.
      const data = await generateRednote(inputType, analysisResult, selectedTone, imitateText, userApiKey);
  ...
  // --- RESULT PAGE ACTIONS ---
  const onRegenerateTitles = async () => { if(!userApiKey){setIsKeyModalOpen(true);return} setIsRegeneratingTitle(true); try{ const t=await regenerateTitles(inputText||"Idea",editableTitle,userApiKey); if(result)setResult({...result,content:{...result.content,titles_options:t}}); }catch(e){alert("失败")} setIsRegeneratingTitle(false); };
  const onRegenerateBody = async () => { if(!userApiKey){setIsKeyModalOpen(true);return} setIsRegeneratingBody(true); try{ const b=await rewriteContent(editableBody,selectedTone==='imitate'?imitateText:"",rewriteInstruction,userApiKey); setEditableBody(b); }catch(e){alert("失败")} setIsRegeneratingBody(false); setShowRewriteModal(false); };
  const onRegenerateParagraph = async (p:string, i:number) => { if(!userApiKey){setIsKeyModalOpen(true);return} setRewritingIndex(i); try{ const t=await rewriteContent(p,selectedTone==='imitate'?imitateText:"","",userApiKey); setEditableBody(prev=>prev.replace(p,t)); }catch(e){alert("失败")} setRewritingIndex(null); };
  const onRegenerateCoverTitle = async () => { if(!userApiKey){setIsKeyModalOpen(true);return} setIsRegeneratingCover(true); try{ const t=await regenerateCoverTitle(editableTitle,editableCoverText,userApiKey); setEditableCoverText(t); }catch(e){alert("失败")} setIsRegeneratingCover(false); };
  const onAskAI = async () => { if(!userApiKey||!askQuestion.trim())return; setIsAsking(true); try{ const a=await askAI(editableBody,askQuestion,userApiKey); setAskAnswer(a); }catch(e){setAskAnswer("错误");} setIsAsking(false); };
    ...
    <div className="min-h-screen font-sans text-gray-300 flex overflow-hidden bg-[#050505]">
      ...
      <aside className="w-64 h-screen flex-col border-r border-white/10 glass-panel z-30 hidden md:flex">
          <div className="p-8 flex items-center gap-3 mb-4">
              ...
              <h1 className="text-lg font-serif font-black tracking-wider text-white">红薯引擎</h1>
          </div>
          
          <nav className="flex-1 px-4 space-y-2">
              <p className="px-4 text-[10px] font-mono font-bold text-gray-600 uppercase tracking-widest mb-2 mt-4">/ 核心模块</p>
              <NavButton type="Type A" label="视频分析" icon={Video} />
              <NavButton type="Type B" label="文档解析" icon={FileText} />
              <NavButton type="Type C" label="热点搜索" icon={Search} />

              <p className="px-4 text-[10px] font-mono font-bold text-gray-600 uppercase tracking-widest mb-2 mt-8">/ 系统设置</p>
              <button onClick={() => setIsKeyModalOpen(true)} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-400 hover:bg-white/5 hover:text-gray-200 transition-all">
                  <Key size={18}/> <span className="font-mono">API 密钥</span>
              </button>
          </nav>

          <div className="p-6 border-t border-white/5">
              <div className="flex items-center gap-3 opacity-60 hover:opacity-100 transition-opacity cursor-pointer">
                  <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-gray-400 font-mono text-xs border border-white/20">用户</div>
                  <div>
                      <p className="text-xs font-bold text-gray-300">专业版会员</p>
      ...
      <main className="flex-1 h-screen overflow-y-auto relative scroll-smooth bg-[#050505] bg-opacity-95">
        ...
        <header className="sticky top-0 z-20 px-4 md:px-8 py-4 flex justify-between items-center border-b border-white/10 bg-[#050505]/80 backdrop-blur-xl">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-lg font-mono font-bold tracking-wider text-white">{step === 'input' ? '// 控制台' : '// 创作工坊'}</h2>
                </div>
            </div>
                        ...
                        <div className="artifact-card p-6 md:p-8 relative overflow-hidden group border border-white/10 rounded-sm">
                            ...
                            <div className="relative z-10">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h3 className="text-3xl md:text-4xl font-serif font-black text-white mb-2 tracking-tight">打造数字<br/>爆款内容</h3>
                                        <p className="text-gray-500 font-mono text-xs md:text-sm mt-2 max-w-md border-l-2 border-neon-lime pl-3">
                                            由 Gemini 3 Pro Preview 驱动. <br/>
                                            深度解析 -> 策略文案生成.
                                        </p>
                                    </div>
                        ...
                        <div className="artifact-card p-6 md:p-8 rounded-sm border border-white/10">
                                     ...
                                     <div className="flex gap-4 flex-col sm:flex-row">
                                         ...
                                         <div className="relative group h-12 sm:h-auto">
                                             ...
                                             <button className="w-full h-full px-6 bg-white/5 border border-white/20 hover:border-white/50 text-gray-300 rounded-none font-mono text-xs flex items-center justify-center gap-2 whitespace-nowrap transition-all">
                                                 {isUploading ? <Loader2 className="animate-spin"/> : <Upload size={16}/>} {videoFile ? "已加载" : "上传文件"}
                                             </button>
                                         </div>
                                     </div>
                                 </div>
                             ...
                             {inputType === 'Type B' && (
                                 <div className="border border-dashed border-white/20 p-12 text-center hover:border-neon-lime/50 hover:bg-neon-lime/5 transition-all cursor-pointer relative group">
                                     ...
                                     {isUploading ? <Loader2 className="mx-auto mb-4 text-neon-lime animate-spin" size={32}/> : <FileText size={32} className="mx-auto mb-4 text-gray-600 group-hover:text-neon-lime transition-colors"/>}
                                     <h4 className="font-mono font-bold text-gray-300 mb-1">{pdfFile ? pdfFile.name : "拖拽 PDF 到此处"}</h4>
                                 </div>
                             )}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="artifact-card p-6 relative overflow-hidden min-h-[200px] border border-white/10">
                            <div className="flex justify-between items-center mb-4 border-b border-white/10 pb-3">
                                <h3 className="text-sm font-mono font-bold text-gray-300 flex items-center gap-2">
                                    <span className="text-neon-lime">02.</span> 智能中枢
                                </h3>
                            ...
                            {analysisResult ? (
                                <div className="space-y-4 text-sm animate-fade-in">
                                    <div className="bg-black border border-white/10 p-4">
                                        <p className="text-[10px] text-neon-lime font-mono mb-2">// 核心摘要</p>
                                        <p className="text-gray-300 font-light leading-relaxed text-xs">{analysisResult.summary}</p>
                                    </div>
                                    <div className="bg-black border border-white/10 p-4">
                                        <p className="text-[10px] text-neon-lime font-mono mb-2">// 关键洞察</p>
                                ...
                                <div className="h-40 flex flex-col items-center justify-center text-gray-700 text-center border border-dashed border-white/5">
                                    ...
                                    <p className="text-[10px] font-mono uppercase">等待数据输入...</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="artifact-card p-6 border border-white/10">
                            <h3 className="text-sm font-mono font-bold text-gray-300 mb-4">风格配置</h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fade-in-up">
                    ...
                    <div className="lg:col-span-5 space-y-6 order-2 lg:order-1">
                        <div className="artifact-card p-6 relative">
                            <div className="absolute top-4 right-4 z-10 bg-black border border-neon-lime px-2 py-1 text-[10px] font-mono font-bold text-neon-lime">实时预览</div>
                            ...
                            <div className="mt-8 space-y-4 border-t border-white/10 pt-6">
                                 ...
                                 <div className="bg-black p-4 border border-white/10 space-y-3">
                                     <div className="flex justify-between items-center">
                                         <label className="text-[10px] font-mono font-bold text-gray-500">封面大字</label>
                                         <button onClick={onRegenerateCoverTitle} className="text-[10px] text-neon-lime font-bold hover:text-white flex items-center gap-1">{isRegeneratingCover ? <Loader2 size={10} className="animate-spin"/> : <Wand2 size={10}/>} AI 优化</button>
                                     </div>
                                     ...
                                     <div className="space-y-2 mt-2">
                                         <label className="text-[10px] font-mono font-bold text-gray-500">核心亮点</label>
                                         ...
                                         <button onClick={addPoint} className="w-full py-2 border border-dashed border-white/10 text-[10px] text-gray-500 hover:text-white hover:border-white/30 flex items-center justify-center gap-1"><Plus size={10}/> 添加一行</button>
                                     </div>

                                     <div className="flex justify-between mt-4 items-center pt-2 border-t border-white/10">
                                         <label className="text-[10px] font-mono font-bold text-gray-500">字号</label>
                    ...
                    <div className="lg:col-span-7 space-y-6 order-1 lg:order-2">
                        ...
                        <div className="artifact-card p-6 border border-white/10">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-sm font-mono font-bold text-gray-300">标题裂变</h3>
                        ...
                        <div className="artifact-card p-6 flex-1 flex flex-col min-h-[600px] border border-white/10">
                            <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
                                <div className="flex gap-2">
                                    <button onClick={() => setShowAskModal(true)} className="px-3 py-1.5 border border-blue-900/50 text-[10px] font-bold text-blue-400 flex items-center gap-1 hover:bg-blue-900/20"><MessageSquare size={12}/> 向 AI 提问</button>
                                    <button onClick={() => setShowRewriteModal(true)} className="px-3 py-1.5 border border-purple-900/50 text-[10px] font-bold text-purple-400 flex items-center gap-1 hover:bg-purple-900/20"><Wand2 size={12}/> 重写</button>
                                </div>
                                <button onClick={() => {navigator.clipboard.writeText(`${editableTitle}\n\n${editableBody}`); setCopied(true); setTimeout(()=>setCopied(false),2000)}} className="text-[10px] font-mono font-bold text-gray-500 hover:text-white flex items-center gap-1 transition-colors">
                                    {copied ? <Check size={12} className="text-neon-lime"/> : <Copy size={12}/>} 一键复制
                                </button>
                            </div>
                                ...
                                <div className="w-full h-full outline-none text-gray-300 leading-8 text-sm md:text-base font-light font-mono" contentEditable suppressContentEditableWarning onBlur={e => setEditableBody(e.currentTarget.innerText)}>
                                    {bodyParagraphs.map((p, i) => (
                                        <div key={i} className="group relative mb-6 hover:bg-white/5 p-2 -mx-2 transition-colors border-l-2 border-transparent hover:border-white/20">
                                            {rewritingIndex === i ? <div className="flex gap-2 text-purple-400 text-sm items-center py-2"><Loader2 className="animate-spin" size={14}/> 正在处理...</div> : <p>{p}</p>}
      ...
      {isKeyModalOpen && (
          <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
              <div className="artifact-card p-8 w-full max-w-md border-l-4 border-neon-lime shadow-[0_0_50px_rgba(0,0,0,1)]">
                  <h3 className="font-serif text-2xl text-white mb-6">系统权限认证</h3>
                  <p className="text-xs font-mono text-gray-500 mb-2">输入 GEMINI API 密钥</p>
                  ...
                  <div className="flex gap-4">
                      <button onClick={() => setIsKeyModalOpen(false)} className="flex-1 py-3 border border-white/20 text-gray-400 font-mono text-xs hover:text-white">取消</button>
                      <button onClick={saveApiKey} className="flex-1 py-3 bg-neon-lime text-black font-bold font-mono text-xs hover:bg-white transition-colors">验证并保存</button>
                  </div>
              </div>
          </div>
      )}
      ...
      {showRewriteModal && (
          <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
              <div className="artifact-card p-8 w-full max-w-md border border-purple-500/30">
                  <h3 className="font-serif text-xl text-white mb-4">重写指令</h3>
                  <textarea className="w-full p-4 bg-black border border-white/20 h-32 mb-6 resize-none focus:border-purple-500 text-sm font-mono text-gray-300" placeholder="> 输入您的重写要求 (例如：更幽默一点)..." value={rewriteInstruction} onChange={e => setRewriteInstruction(e.target.value)} />
                  <div className="flex justify-end gap-3">
                      <button onClick={() => setShowRewriteModal(false)} className="px-6 py-3 text-gray-500 font-mono text-xs hover:text-white">取消</button>
                      <button onClick={onRegenerateBody} className="px-6 py-3 bg-purple-600 text-white font-bold font-mono text-xs hover:bg-purple-500">执行重写</button>
                  </div>
              </div>
          </div>
      )}
           ...
           <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
              <div className="artifact-card p-6 w-full max-w-lg border border-blue-500/30">
                  <div className="flex justify-between items-center mb-6">
                      <h3 className="font-bold text-white flex items-center gap-2"><BrainCircuit className="text-blue-500"/> 智能问答核心</h3>
                  ...
                  <div className="flex gap-2">
                      <input className="flex-1 p-3 bg-black border border-white/20 text-sm font-mono text-white focus:border-blue-500 outline-none" placeholder="> 输入您的问题..." value={askQuestion} onChange={e => setAskQuestion(e.target.value)} onKeyDown={e => e.key === 'Enter' && onAskAI()} />
